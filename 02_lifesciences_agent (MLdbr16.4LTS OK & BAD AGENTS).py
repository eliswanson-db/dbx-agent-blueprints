...
# DBTITLE 1,register bad agent
mlflow.set_registry_uri("databricks-uc")

model_name = "orchestrator_bad_agent"
UC_MODEL_NAME = f"{catalog}.{schema}.{model_name}"
...
# Register the model - this creates a new version
print(f"Registering model from run: {logged_bad_agent_info.run_id}")
uc_registered_model_info = mlflow.register_model(
    model_uri=logged_bad_agent_info.model_uri, name=UC_MODEL_NAME
)

print(f"Model registered: {UC_MODEL_NAME} (version {uc_registered_model_info.version})")
print(f"  Run ID: {logged_bad_agent_info.run_id}")
print(f"  Model URI: {logged_bad_agent_info.model_uri}")
