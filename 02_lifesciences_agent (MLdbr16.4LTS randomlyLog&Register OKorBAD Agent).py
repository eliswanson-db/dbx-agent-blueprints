...
dbutils.widgets.text(
    "model_name", "mmt.LS_agent.lifesciences_agent", "FQ Model Name (UC)"
)

dbutils.widgets.text("vs_endpoint_name", "lifesciences_vector_search", "VectorSearch_endpoint")
...
print(f"FQ Model Name: {model_name}")
...
# MAGIC %md
# MAGIC ## Randonly select between OK or BAD Agent Definition --> Log the Agent as an MLflow Model
...
# Randomly select between 'agent.py' and 'bad_agent.py'
selected_agent_file = random.choice(['agent.py', 'bad_agent.py'])
...
# MAGIC %md
# MAGIC ## Create a “bad” agent implementation
# MAGIC In your Databricks notebook, create a variant of agent.py with intentionally broken behavior. Easiest: keep the same interface (AGENT is a ResponsesAgent), but override the LangGraph to always reply with something useless.
# MAGIC

# COMMAND ----------

# DBTITLE 1,bad_agent.py
# MAGIC %%writefile bad_agent.py
...
import bad_agent  # this executes bad_agent.py and registers BAD_AGENT
...
# Log the bad agent model
with mlflow.start_run():
    logged_bad_agent_info = mlflow.pyfunc.log_model(
        name="bad_agent",
        python_model="bad_agent.py",
        pip_requirements=[
            "databricks-langchain",
            "databricks-vectorsearch",
            "psycopg[binary]",  # Required by databricks-langchain for checkpoint functionality
...
print(f"Bad Agent logged: {logged_bad_agent_info.model_uri}")
...
# DBTITLE 1,uc_registered_model_infom
uc_registered_model_info

# COMMAND ----------

# MAGIC %md
# MAGIC ### Extract uc_registered_model_info from code
...
FQ_MODEL_NAME = "mmt.ls_agent.orchestrator_bad_agent"  # Use your actual model name

client = MlflowClient()

# Get the registered model info
try:
    registered_model = client.get_registered_model(FQ_MODEL_NAME)
    print(f"Model name: {registered_model.name}")
    print(f"Description: {getattr(registered_model, 'description', '')}")
    print(f"Creation timestamp: {getattr(registered_model, 'creation_timestamp', '')}")
    print(f"Last updated: {getattr(registered_model, 'last_updated_timestamp', '')}")
    print(f"User ID: {getattr(registered_model, 'user_id', 'N/A')}")
    print(f"Tags: {getattr(registered_model, 'tags', {})}")
    # RegisteredModel aliases (if available)
    print(f"Aliases: {getattr(registered_model, 'aliases', 'N/A')}")
except Exception as e:
    print(f"Could not fetch model: {e}")

# Get all versions for this model and their aliases
try:
    versions = client.search_model_versions(f"name='{FQ_MODEL_NAME}'")
    # Build a mapping of version -> aliases
    alias_map = {}
    # List all aliases for this model and which version they point to
    try:
        aliases = client.get_registered_model_aliases(FQ_MODEL_NAME)
        for alias in aliases:
            alias_map.setdefault(alias.version, []).append(alias.alias)
    except Exception:
        pass  # Aliases may not be present

    for v in versions:
        print(f"\nVersion: {v.version}")
        print(f"  Status: {getattr(v, 'current_stage', None)}")
        print(f"  Run ID: {getattr(v, 'run_id', None)}")
        print(f"  Creation timestamp: {getattr(v, 'creation_timestamp', None)}")
        print(f"  Source: {getattr(v, 'source', None)}")
        # Print aliases for this version, if any
        print(f"  Aliases: {alias_map.get(v.version, [])}")
except Exception as e:
